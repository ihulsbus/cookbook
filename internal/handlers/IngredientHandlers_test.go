...
func TestIngredientDelete_OK(t *testing.T) {
	...
	req := httptest.NewRequest("DELETE", "http://example.com/api/v1/ingredient/1", bytes.NewReader(reqBody))
	w := httptest.NewRecorder()
...
func TestIngredientDelete_UnmarshalErr(t *testing.T) {
	...
	req := httptest.NewRequest("DELETE", "http://example.com/api/v1/ingredient/1", bytes.NewReader([]byte{}))
	w := httptest.NewRecorder()
